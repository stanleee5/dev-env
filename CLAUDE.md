...
- The LSP layer is native (`vim.lsp.config`/`vim.lsp.enable`, nvim 0.11+) with `nvim-lspconfig` providing server definitions and `blink.cmp` providing completion. All LSP setup — server settings, diagnostics, keymaps — lives in `config/nvim/lua/plugins/lsp.lua`. Servers: `pyright` (Python types) and `bash-language-server` (npm, via `install_language_servers` in `install_nvim.sh`), `ruff` (Python lint/format; from `config/requirements.txt` via pip), and `lua-language-server` (GitHub release tarball, Linux x64 only — macOS hosts use brew). A server is only enabled if its binary is on PATH; `shellcheck` (apt) gives bashls linting. `config/nvim/.luarc.json` is the lua_ls root marker — without one lua_ls runs in single-file mode and publishes no diagnostics (and note `copy_config` uses `cp -r "$src"/.` so dotfiles like it get copied).
